//
//  managing_memory_with_pointers.c
//  CCourse
//
...
typedef struct CoffeeMachine {
    int (*pour)(long, struct CoffeeMachine*);
    Metric **metrics;
    long pour_duration;
} CoffeeMachine;
...
int pour_decaf(long, CoffeeMachine*);
int pour_classic(long, CoffeeMachine*);
int pour_rich(long, CoffeeMachine*);
...
int Memory_Management(int argc, char *argv[]) {
    ...
    printf("\nBeginning Pour With Duration: %ld\n", duration);
...
int pour_decaf(long duration, CoffeeMachine* machine) {
    int start = 0;
...
int pour_classic(long duration, CoffeeMachine *machine) {
    int start = 0;
    Metric **metrics_ptr = (Metric**)malloc(duration * sizeof(Metric));
    machine->metrics = metrics_ptr;
...
int pour_rich(long duration, CoffeeMachine *machine) {
    int start = 0;
    Metric **metrics_ptr = (Metric**)malloc(duration * sizeof(Metric));
    machine->metrics = metrics_ptr;
